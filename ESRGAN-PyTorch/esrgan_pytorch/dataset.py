...
class BaseTrainDataset(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, root: str, image_size: int = 128, upscale_factor: int = 4):
        ...
        self.filenames = [
            os.path.join(root, x) for x in os.listdir(root) if check_image_file(x)
        ]
        ...
        self.hr_transforms = transforms.Compose(
            [transforms.RandomCrop((image_size, image_size)), transforms.ToTensor()]
        )
...
class CustomTrainDataset(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, root: str, sampler_frequency: int = 1):
        ...
        self.sampler_filenames = random.sample(
            self.filenames, len(self.filenames) // sampler_frequency
        )
        self.lr_filenames = [
            os.path.join(lr_dir, x)
            for x in self.sampler_filenames
            if check_image_file(x)
        ]
        self.hr_filenames = [
            os.path.join(hr_dir, x)
            for x in self.sampler_filenames
            if check_image_file(x)
        ]
