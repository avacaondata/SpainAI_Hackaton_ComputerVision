...
from .image_quality_assessment import GMSD, LPIPS, SSIM

__all__ = ["iqa", "test"]


def iqa(
    image1_tensor: torch.Tensor, image2_tensor: torch.Tensor, gpu: int = None
) -> [
    torch.Tensor,
    torch.Tensor,
    torch.Tensor,
...
def test(
    model: nn.Module, dataloader: torch.utils.data.DataLoader, gpu: int = None
    ...
    with torch.no_grad():
        for i, (lr, hr) in progress_bar:
            ...
            total_psnr_value += 10 * torch.log10(1.0 / mse_loss(sr, hr))
    ...
    out = (
        total_psnr_value / total,
        total_ssim_value / total,
        total_lpips_value / total,
        total_gmsd_value / total,
    )
